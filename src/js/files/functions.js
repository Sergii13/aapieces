...
export function tabs() {
   ...
   function setTabsStatus2(tabsBlock) {
      let tabsTitles = tabsBlock.querySelectorAll('.select__option');
      let tabsContent = tabsBlock.querySelectorAll('[data-tabs-item]');
      const tabsBlockIndex = tabsBlock.dataset.tabsIndex;
   ...
   function setTabsAction(e) {
      ...
      if (el.closest('[data-tabs-title]')) {
         ...
         e.preventDefault();
      } else if (el.closest('.select__option') && el.closest('[data-mob-select]')) {
         const tabTitle = el.closest('.select__option');
         const tabsBlock = tabTitle.closest('[data-tabs]');
         if (!tabTitle.classList.contains('_tab-active') && !tabsBlock.querySelector('._slide')) {
            let tabActiveTitle = tabsBlock.querySelectorAll('.select__option._tab-active');
            tabActiveTitle.length ? tabActiveTitle = Array.from(tabActiveTitle).filter(item => item.closest('[data-tabs]') === tabsBlock) : null;
            tabActiveTitle.length ? tabActiveTitle[0].classList.remove('_tab-active') : null;
            tabTitle.classList.add('_tab-active');
...
if (document.querySelector("[data-filter]")) {
   document.addEventListener("click", function (e) {
      if (bodyLockStatus && e.target.closest('[data-filter]')) {
         bodyLockToggle();
         document.documentElement.classList.toggle("filter-open");
      }
   });
};
if (document.querySelector("[data-close-filter]")) {
   document.addEventListener("click", function (e) {
      if (bodyLockStatus && e.target.closest('[data-close-filter]')) {
         bodyLockToggle();
